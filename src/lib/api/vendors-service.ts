 ...
 */
async function fetchWithNextUrl(
  nextUrl: string,
  apiToken: string
...
/**
 * Create a Vendors Service bound to a specific client
 */
export function createVendorsService(client: GetProvenClient, apiToken: string) {
  return {
    /**
    ...
    async getVendors(
      ...
      try {
        ...
        if (nextUrl) {
          ...
          response = await fetchWithNextUrl(nextUrl, apiToken);
        } else {
          // Initial fetch with page and page_size
          response = await client.getVendors({
            page,
            pageSize,
    ...
    async getVendorById(id: string): Promise<ApiResponse<GetProvenVendor>> {
      try {
        ...
        const response = await client.getVendors({ pageSize: 500 });
    ...
    async getVendorClients(vendorId: string): Promise<ApiResponse<VendorClient[]>> {
      try {
        const response = await client.getVendorClients(vendorId, 50);
    ...
    async getVendorContacts(vendorId: string): Promise<ApiResponse<VendorUser[]>> {
      try {
        const response = await client.getVendorUsers(vendorId);
...
export type VendorsService = ReturnType<typeof createVendorsService>;
