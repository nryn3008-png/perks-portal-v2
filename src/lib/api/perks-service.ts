...
function logApiError(operation: string, error: unknown): void {
  if (typeof window === 'undefined') {
    logger.error(`[Perks Service] ${operation} failed:`, {
...
/**
 * Create a Perks Service bound to a specific client
 */
export function createPerksService(client: GetProvenClient, apiToken: string) {
  return {
    /**
     * Get offers from API
    ...
    async getOffers(
      ...
      try {
        ...
        if (nextUrl) {
          ...
          response = await client.getDeals({
            page,
            pageSize,
    ...
    async getFilterOptions(): Promise<{
      ...
      try {
        ...
        const response = await client.getDeals({ pageSize: 500 });
...
export type PerksService = ReturnType<typeof createPerksService>;
